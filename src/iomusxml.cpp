    ...
    pugi::xml_node node, Measure *measure, const std::string &measureNum, const short int staffOffset, Section *section)
{
        ...
        for (pugi::xml_node technical : notations.node().children("technical")) {
            for (pugi::xml_node technicalChild : technical.children()) {
                ...
                if (technicalChildName == "fingering") {
                    ...
                    const bool hasValidNoteStaff = (noteStaffNum > 0) && (noteStaffNum <= measure->GetStaffCount());
                    const int fingeringStaff = hasValidNoteStaff ? noteStaffNum + staffOffset : staff->GetN();
                    fing->SetStaff(staff->AttNInteger::StrToXsdPositiveIntegerList(std::to_string(fingeringStaff)));
                    data_STAFFREL place
    ...
    if (xmlBreath) {
        ...
        breath->SetTstamp((double)elementEndTime * (double)m_meterUnit / (double)(4 * m_ppq) + 0.9);
    }
    ...
    if (xmlCaesura) {
        ...
        caesura->SetTstamp((double)elementEndTime * (double)m_meterUnit / (double)(4 * m_ppq) + 0.9);
    }
